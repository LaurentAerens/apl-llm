...
def load_model_v2(checkpoint_path: str = "checkpoints/apl_slm_best.pt", device_str: str = None) -> Tuple[APL_SLM_v2, APLTokenizer, torch.device]:
...
def autocomplete_v2(
    ...
    if verbose:
        print(f"\n[+] Prompt: {repr(prompt)}")
        print("[+] Autocomplete v2: ", end="", flush=True)
        ...
        if use_kv_cache:
            ...
            for _ in range(max_new_tokens):
                ...
                if guard_syntax and cur_depth == 0:
                    # Discourage unmatched closers when depth is 0
                    for ch in (")", "]", "}"):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="APL SLM v2 Autocomplete")
    parser.add_argument("--checkpoint", type=str, default="checkpoints/Small-v2/apl_slm_best.pt")
    ...
    model, tokenizer, device = load_model_v2(args.checkpoint)
    completed = autocomplete_v2(
