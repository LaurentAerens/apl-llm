...
class APLDataset_v2(Dataset):
    """Chunked sequence dataset for causal language modeling with depth conditioning (v2)."""
...
def train_v2(args):
    ...
    exp_name = getattr(args, "exp_name", None) or "Small-v2"
    ...
    depth_loss_weight = getattr(args, "depth_loss_weight", 0.2)
    ...
    suffix = "_demo.pt" if getattr(args, "demo", False) else ".pt"
    corpus_stem = data_path.stem
    tokens_path = Path("data") / f"{corpus_stem}_tokens{suffix}"
    depths_path = Path("data") / f"{corpus_stem}_depths{suffix}"

    full_dataset = APLDataset_v2(
        text=corpus_text,
        tokenizer=tokenizer,
    ...
    print(f"[+] Initialized APL_SLM v2 ({n_layer}L, {n_head}H, {n_embd}D). Parameters: {model.count_parameters():,}")
    ...
    print("\n" + "=" * 60)
    print(f"🚀 Starting APL SLM v2 Training Loop ({exp_name})")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_idx, (x_tok, y_tok, x_depth, y_depth) in enumerate(train_loader):
            ...
            with torch.amp.autocast(device_type=device.type, dtype=autocast_dtype, enabled=use_autocast):
                logits, depth_logits, _ = model(x_tok, x_depth)
                ...
                raw_loss = loss_tok + depth_loss_weight * loss_depth
        ...
        with torch.no_grad():
            for x_tok, y_tok, x_depth, y_depth in val_loader:
                ...
                with torch.amp.autocast(device_type=device.type, dtype=autocast_dtype, enabled=use_autocast):
                    ...
                    loss = l_tok + depth_loss_weight * l_depth
        ...
        with open(history_path, "w", encoding="utf-8") as f:
            json.dump({"experiment": exp_name, "version": 2, "epochs": history}, f, indent=2)
    ...
    print(f"\n[OK] v2 Training Complete in {elapsed:.2f}s! Best Validation Loss: {best_val_loss:.4f}")


def main():
    parser = argparse.ArgumentParser(description="Train APL SLM v2 (Depth Conditioned)")
    parser.add_argument("--model_version", "--version", "-v", dest="version", type=int, default=2)
    parser.add_argument("--data_file", "--dataset_path", dest="data_file", type=str, default="data/apl_corpus.txt")
    parser.add_argument("--exp_name", type=str, default="Small-v2")
