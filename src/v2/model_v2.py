...
class Block_v2(nn.Module):
    def __init__(self, config: APL_SLMConfig_v2):
        ...
        self.attn = CausalSelfAttention_v2(config)
        self.ln_2 = nn.LayerNorm(config.n_embd)
        self.mlp = MLP_v2(config)
...
class APL_SLM_v2(nn.Module):
    """
    Architecture Version 2: Structural Depth Conditioned Transformer for APL.
    Conditioned on running parenthesis, bracket, and dfn depth with auxiliary depth prediction head.
    """
    ...
    def __init__(self, config: APL_SLMConfig_v2):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                wde=nn.Embedding(config.max_depth, config.n_embd) if config.use_depth_node else None,
                drop=nn.Dropout(config.dropout),
                h=nn.ModuleList([Block_v2(config) for _ in range(config.n_layer)]),
