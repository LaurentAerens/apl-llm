...
def train_v3(args):
    ...
    exp_name = getattr(args, "exp_name", None) or "Small-v3"
    ...
    print(f"[+] Initialized APL_SLM v3 ({n_layer}L, {n_head}H, {n_embd}D). Parameters: {model.count_parameters():,}")
    ...
    print("\n" + "=" * 60)
    print(f"🚀 Starting APL SLM v3 Training Loop ({exp_name})")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        with open(history_path, "w", encoding="utf-8") as f:
            json.dump({"experiment": exp_name, "version": 3, "epochs": history}, f, indent=2)
    ...
    print(f"\n[OK] v3 Training Complete in {elapsed:.2f}s! Best Validation Loss: {best_val_loss:.4f}")


def main():
    parser = argparse.ArgumentParser(description="Train APL SLM v3 (Modern RoPE+SwiGLU)")
    parser.add_argument("--model_version", "--version", "-v", dest="version", type=int, default=3)
